...
class WeekdayFood:
    def __init__(self):
        self.choice = [
            'Chick-fil-A', 'Moe\'s', 'Panda Express',
            'Cafeteria', 'Burger 256', 'CREATE', 'Lions Corner'
        ]
...
class WeekendFood(WeekdayFood):
    def __init__(self):
        self.choice = [
            'Chick-fil-A', 'Cafeteria', 'CREATE', 'Starbucks'
        ]
        self.rand_index = random.randint(0, len(self.choice) - 1)
...
class MyWindow(QMainWindow):
    ...
    def display_food(self):
        ...
        elif datetime.today().weekday() < 5 and current_month >= 8 and current_month <= 4:
            weekday_food = WeekdayFood()
            self.label.setText(str(weekday_food))
        # checks if it's a weekend and if the month is between august and april
        else:
